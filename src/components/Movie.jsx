...
export default function Movie() {
    ...
    // useMovieSearch(id, setMovie) 

    useEffect(() => {
        async function fetchMovie() {
            try {
                const response = await fetch(`https://omdbapi.com/?apikey=3df3b4a8&i=${id}`);
                if (!response.ok) {
    ...
    const { imdbID, Title, Poster, Plot, Runtime, Year, Ratings } = movieData;
    ...
    return (
        <div>
            <article className="movie" key={imdbID}>
                ...
                {Poster !== 'N/A' && <img src={Poster} alt={`Filmplakat ${Title}`} className="movie__poster" />}
                ...
                <dl className="movie__details">
                    ...
                    {Runtime && (
                        <>
                            <dt>Dauer</dt>
                            <dd>{Runtime.replace("min", "")} Minuten</dd>
                        </>
                    )}
                </dl>
                {Ratings.length > 0 &&
...
// CUSTOM HOOK
// function useMovieSearch(id, setMovieData) {
//     useEffect(() => {
//         async function fetchMovie() {
//             try {
